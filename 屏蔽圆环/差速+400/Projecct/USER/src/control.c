    ...
    while(1)
    {
        ...
        if (mt9v032_finish_flag == 1)
        {
            ...
            switch(current_element)
            {
              case CURVE:
                if(barrier_flag){
                    if(valid_row>30)//&&myabs(direction_error)<10)//&&average_error<10)//&&length>500&&length<1000)
                    {
                        Control_Barrier();
                    }
                    barrier_flag = 0;
...
int different=0;
PID_TypeDef DifferentSpeed_PID={1200,0,0,0,0,0,0};
void Different_Speed()
{
    if(myabs(average_error)>10)
    {
        if(speed_mode<=2)
            DifferentSpeed_PID.Kp = 800;
        else
            DifferentSpeed_PID.Kp = 800;
...
void Control_LeftSmallRoundabout(){
    if (round_state == 1){
        ...
        if (r_jump_temp>0 && r_jump_temp>30 && white>(119-r_jump_temp) * 3 / 4){//走到岔路口固定打角直接进圆环
            ...
            direction_error = limit(right_points[Round_Direction_Judge_Row] - track_width[Round_Direction_Judge_Row] / 2 - img_width / 2, 0, 187);
            Control_Servo(Servo_PID_Curve);//舵机pid控制
        }
...
void Control_RightSmallRoundabout(){
    if (round_state == 1){
        ...
        if (r_jump_temp>0 && r_jump_temp>30 && white>(119-r_jump_temp) * 3 / 4){//走到岔路口补线固定打角直接进圆环
            ...
            direction_error = limit(left_points[Round_Direction_Judge_Row] + track_width[Round_Direction_Judge_Row] / 2 - img_width / 2, 0, 187);
            Control_Servo(Servo_PID_Curve);//舵机pid控制
        }
        OLED_Print_Num1(64, 2, round_state);
    }
