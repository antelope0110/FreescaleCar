...
void camera_init(void)
{
    ...
    //摄像头采集初始化
    DisableInterrupts;
    //DMA通道0初始化，PTB6触发源(默认上升沿)，源地址为D_IN_DATA(1)(PTD8-PTD15)，目的地址为：image，每次传输1Byte 传输完毕保持目的地址
    dma_portx2buff_init(DMA_CH0, (void *)&D_IN_DATA(1), (void *)image, B6, DMA_BYTE1, COL*ROW, DADDR_KEEPON);
    port_init(B6, ALT1 | DMA_RISING | PULLDOWN);  			    //PCLK  触发源设置
    ...
    port_init(B7, ALT1 | IRQ_FALLING | PULLDOWN);        		//PTB7场中断，下降沿触发中断、下拉
    set_irq_priority(PORTB_IRQn,1);                             // 中断优先级
...
void VSYNC(void)
{
	if (!mt9v032_finish_flag) dma_repeat(DMA_CH0, (void *)&D_IN_DATA(1), (void *)image[0], COL*ROW);
}
...
{
	uart_putchar(CAMERA_UART,0x00);
	uart_putchar(CAMERA_UART,0xff);
	uart_putchar(CAMERA_UART,0x01);
	uart_putchar(CAMERA_UART,0x01);//发送命令
    uart_putbuff(CAMERA_UART, (uint8 *)image, ROW*COL);  //发送图像
}
